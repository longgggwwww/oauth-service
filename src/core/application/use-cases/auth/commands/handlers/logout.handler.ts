...
export class LogoutHandler implements ICommandHandler<LogoutCommand> {
    ...
    async execute(command: LogoutCommand): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Failed to revoke refresh tokens for user ${userId}: ${error.message}`, error.stack);
        ...
        if (sessionId) {
            try {
                ...
                this.logger.warn(`Failed to delete auth session ${sessionId}: ${error.message}`);
            }
        }
