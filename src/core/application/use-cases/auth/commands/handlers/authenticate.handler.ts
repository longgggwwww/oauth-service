...
import { InvalidCredentialsException, UserNotFoundException } from '@src/core/domain/exceptions/domain-exceptions';
...
export class AuthenticateHandler implements ICommandHandler<AuthenticateCommand> {
    ...
    async execute(command: AuthenticateCommand): Promise<any> {
        ...
        if (clientId) {
            ...
            if (clientSecret) {
                const isClientValid = await this.cryptoService.verifySecret(clientSecret, client.clientSecret);
                if (!isClientValid) {
        ...
        // 3. Verify password
        if (password) {
            const isValid = await this.cryptoService.verifySecret(password, credential.passwordHash);
            if (!isValid) {
                throw new InvalidCredentialsException();
            }
        } else {
