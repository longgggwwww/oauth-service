...
export class ExchangeTokenHandler implements ICommandHandler<ExchangeTokenCommand> {
    ...
    async execute(command: ExchangeTokenCommand): Promise<any> {
        ...
        switch (request.grant_type) {
            ...
            default:
                throw new BadRequestException(`Unsupported grant type: ${request.grant_type}`);
        }
    }
     ...
     */
    private async handleRefreshToken(refreshTokenStr: string, client: any): Promise<any> {
        if (!refreshTokenStr) {
        ...
        const storedToken = await this.refreshTokenRepository.findByToken(refreshTokenStr);
        if (!storedToken) {
     ...
     */
    private async handleAuthorizationCode(request: any, client: any): Promise<any> {
        throw new BadRequestException('Authorization code flow not yet implemented');
        // TODO: Verify authorization code, code_verifier, redirect_uri
