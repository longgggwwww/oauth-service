...
Danh sách các endpoint chính trong hệ thống (dựa trên mã nguồn, tất cả có prefix `/v1`).

> **Lưu ý:** Các endpoint bị ~~gạch ngang~~ là các API liên quan đến workflow authorization code hoặc quản lý user, không cần thiết cho hệ thống chỉ tập trung vào grant type `client_credentials`. 
Các endpoints trên sẽ mở trở lại khi có cập nhật
